...
@section('additionals', 'active')
@section('title', 'عرض الأضافى الشهر المالى')
@section('css')
...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h5 class="content-title mb-0 my-auto">بيانات الأضافى الرواتب للشهر
                    المالى {{ $finance_cln_periods_data->month->name }}</h5><span class="text-muted mt-1 tx-13 mr-2 mb-0">/
                    <a style="font-weight:bold;color:#117ea1;" href="{{ route('dashboard.additionals.index') }}">
                        <i class="fas fa-arrow-circle-left fa-lg ml-1"></i> سجلات الأضافى الموظفين
                    </a>
                </span>
            </div>
        </div>
                ...
                <div class="mb-3 mb-xl-0">
                    <div class="btn-group dropdown">
                        <a class="modal-effect btn btn-outline-primary btn-block" data-effect="effect-super-scaled"
                            data-toggle="modal" href="#modaldemo8"> <i class="fas fa-plus-circle"></i> أنشاء أضافى جديد
                            للموظف</a>
                    </div>
                </div>
            </div>
...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-0">
                            بيانات الأضافى الرواتب للشهر المالى (<span
                ...
                <form method="POST" action="{{ route('dashboard.additionals.print_search') }}" target="_blank">
                    @csrf
                                            ...
                                            <tr>
                                                ...
                                                @if ($finance_cln_periods_data['is_open'] == 1)
                                                    <td>
                                                        ...
                                                        @include('dashboard.salaries.additionals.delete')
                                                    </td>
                                                @endif
                                            </tr>
                                        @endforeach
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">أضافة الأضافى للموظفين للشهر المالى <span class="text-danger">(
                ...
                <div class="modal-body" id="addModalBody">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>أجمالى قيمة الأضافى</label>
                                <input autofocus type="text" oninput="this.value=this.value.replace(/[^0-9.]/g,'');"
                                    readonly name="total_add" id="total_add" class="form-control" value="">
                                @error('total')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
    ...
    <div class="modal fade edit" id="EditModal">
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">تعديل الأضافى للموظفين للشهر المالى <span class="text-danger">(
                            {{ $finance_cln_periods_data->year_and_month }} )</span> </h6>
                    <button aria-label="Close" class="close" data-dismiss="modal" type="button"><span
                            aria-hidden="true">&times;</span></button>
                </div>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '#do_add_now', function(e) {
                ...
                if (value_add === "") {
                    ...
                    Swal.fire({
                        icon: 'warning',
                        title: 'تحذير',
                        text: 'من فضلك ادخل عدد ايام الأضافى',
                        customClass: {
                            container: 'swal2-override'
                        }
                    }).then((result) => {
                ...
                if (total_add === "") {
                    ...
                    Swal.fire({
                        icon: 'warning',
                        title: 'تحذير',
                        text: 'من فضلك ادخل اجمالي الأضافى',
                        customClass: {
                            container: 'swal2-override'
                        }
                    }).then((result) => {
                ...
                // عمل فحص قبل التخزين
                $.ajax({
                    url: '{{ route('dashboard.additionals.checkExsistsBefor') }}',
                    type: 'post',
                    dataType: 'json',
                    cache: false,
                    data: {
                    ...
                    success: function(data) {
                        if (data == 'exsists_befor') {
                            ...
                            Swal.fire({
                                title: 'تحذير',
                                text: 'يوجد سجل غياب سابق مسجل للموظف هل تريد الاستمرار؟',
                                icon: 'warning',
                    ...
                    error: function(xhr, status, error) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'خطأ',
                            text: 'عفوا لقد حدث خطأ في التحقق من الأضافىات السابقة',
                            customClass: {
                                container: 'swal2-override'
                            }
                        }).then(() => {
            ...
            function submitSanction() {
                ...
                $.ajax({
                    url: '{{ route('dashboard.additionals.store') }}',
                    type: 'post',
                    dataType: 'html',
                    cache: false,
                    data: {
    ...
    <script>
        ...
        function ajax_search() {
            ...
            jQuery.ajax({
                url: '{{ route('dashboard.additionals.ajax-search') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
    ...
    <script>
        ...
        $(document).on('click', '.load_edit_this_row', function(e) {
            ...
            jQuery.ajax({
                url: '{{ route('dashboard.additionals.load_edit_row') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
        ...
        $(document).on('click', '#do_edit_now', function(e) {
            ...
            if (value_edit === "") {
                return showWarning('من فضلك ادخل عدد ايام الأضافى', '#value_edit');
            }
            $('#EditModal').modal('show'); // إظهار الـ modal مرة أخرى
            ...
            if (total_edit === "") {
                return showWarning('من فضلك ادخل اجمالي الأضافى', '#total_edit');
            }
            $('#EditModal').modal('show'); // إظهار الـ modal مرة أخرى
            ...
            jQuery.ajax({
                url: '{{ route('dashboard.additionals.do_edit_row') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
