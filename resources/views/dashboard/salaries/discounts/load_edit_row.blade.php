    ...
    <div class="row">
        <div class="col-md-6">
            <div class="form-group">
                ...
                <select name="discount_types_id_edit" id="discount_types_id_edit" class="form-control select2" style="width: 100%;">
                    <option value="" disabled>-- اختر نوع الخصومات --</option>
                    @if (@isset($discounts) && !@empty($discounts))
                        @foreach ($discounts as $dis)
                            <option @if ($sanctions['discount_types_id'] == $dis->id) selected @endif value="{{ $dis->id }}">
                                {{ $dis->name }}</option>
                        @endforeach
                    @else
                        <div class="alert alert-warning" role="alert" dir="rtl">
