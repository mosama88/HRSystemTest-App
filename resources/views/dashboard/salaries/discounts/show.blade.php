...
@section('discounts', 'active')
@section('title', 'عرض الخصومات الشهر المالى')
@section('css')
...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h5 class="content-title mb-0 my-auto">بيانات الخصومات الرواتب للشهر
                    المالى {{ $finance_cln_periods_data->month->name }}</h5><span class="text-muted mt-1 tx-13 mr-2 mb-0">/
                        <a style="font-weight:bold;color:#117ea1;" href="{{ route('dashboard.discounts.index') }}">
                            <i class="fas fa-arrow-circle-left fa-lg ml-1"></i> سجلات الخصومات الموظفين
                        </a>
                </span>
            </div>
        </div>
                ...
                <div class="mb-3 mb-xl-0">
                    <div class="btn-group dropdown">
                        <a class="modal-effect btn btn-outline-primary btn-block" data-effect="effect-super-scaled"
                            data-toggle="modal" href="#modaldemo8"> <i class="fas fa-plus-circle"></i> أنشاء خصومات جديد
                            للموظف</a>
                    </div>
                </div>
            </div>
...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-0">
                            بيانات الخصومات الرواتب للشهر المالى (<span
                ...
                <form method="POST" action="{{ route('dashboard.discounts.print_search') }}" target="_blank">
                    @csrf
                    ...
                    <div class="col-10 mx-auto mt-4">
                        ...
                        <div class="row">
                            ...
                            <div class=" col-3">
                                <div class="form-group">
                                    <label> بحث بنوع الخصومات </label>
                                    <select name="discount_types_id_search" id="discount_types_id_search"
                                        class="form-control">
                                        <option value="all">بحث بالكل</option>
                                            ...
                                            <tr>
                                                ...
                                                @if ($finance_cln_periods_data['is_open'] == 1)
                                                    <td>
                                                        ...
                                                        @include('dashboard.salaries.discounts.delete')
                                                    </td>
                                                @endif
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">أضافة الخصومات للموظفين للشهر المالى <span class="text-danger">(
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="discount_types_id_add" id="discount_types_id_add"
                                    class="form-control select2" style="width: 100%;">
                                    <option value="">-- اختر نوع الخصومات --</option>
                                    @if (@isset($discounts) && !@empty($discounts))
                                        @foreach ($discounts as $dis)
                                            <option value="{{ $dis->id }}">{{ $dis->name }}</option>
                                        @endforeach
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input autofocus type="text" oninput="this.value=this.value.replace(/[^0-9.]/g,'');"
                                    name="total_add" id="total_add" class="form-control" value="">
                                @error('total')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
    ...
    <div class="modal fade edit" id="EditModal">
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">تعديل الخصومات للموظفين للشهر المالى <span class="text-danger">(
                            {{ $finance_cln_periods_data->year_and_month }} )</span> </h6>
                    <button aria-label="Close" class="close" data-dismiss="modal" type="button"><span
                            aria-hidden="true">&times;</span></button>
                </div>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '#do_add_now', function(e) {
                ...
                var discount_types_id_add = $("#discount_types_id_add").val();
                if (discount_types_id_add === "") {
                    $('#modaldemo8').modal('hide'); // إخفاء الـ modal
                    Swal.fire({
                        icon: 'warning',
                        title: 'تحذير',
                        text: 'من فضلك اختر نوع الخصومات',
                        customClass: {
                            container: 'swal2-override'
                        }
                    }).then((result) => {
                ...
                if (total_add === "" || total_add == 0) {
                    ...
                    Swal.fire({
                        icon: 'warning',
                        title: 'تحذير',
                        text: 'من فضلك ادخل اجمالي الخصومات للموظف',
                        customClass: {
                            container: 'swal2-override'
                        }
                    }).then((result) => {
                ...
                // عمل فحص قبل التخزين
                $.ajax({
                    url: '{{ route('dashboard.discounts.checkExsistsBefor') }}',
                    type: 'post',
                    dataType: 'json',
                    cache: false,
                    data: {
                    ...
                    success: function(data) {
                        if (data == 'exsists_befor') {
                            ...
                            Swal.fire({
                                title: 'تحذير',
                                text: 'يوجد سجل اضافى سابق مسجل للموظف هل تريد الاستمرار؟',
                                icon: 'warning',
                    ...
                    error: function(xhr, status, error) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'خطأ',
                            text: 'عفوا لقد حدث خطأ في التحقق من الالخصومات السابقة',
                            customClass: {
                                container: 'swal2-override'
                            }
                        }).then(() => {
            ...
            function submitAdditional() {
                ...
                $.ajax({
                    url: '{{ route('dashboard.discounts.store') }}',
                    type: 'post',
                    dataType: 'html',
                    cache: false,
                    data: {
                        ...
                        discount_types_id: discount_types_id_add,
                        total: total_add,
                        notes: notes_add,
                        day_price: day_price,
                    },
    ...
    <script>
        ...
        function ajax_search() {
            ...
            var discount_types_id = $("#discount_types_id_search").val();
            var is_archived = $("#is_archived_search").val();
            var the_finance_cln_periods_id = $("#the_finance_cln_periods_id").val();

            jQuery.ajax({
                url: '{{ route('dashboard.discounts.ajax-search') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
                    ...
                    discount_types_id: discount_types_id,
        ...
        $('#employee_code_search, #discount_types_id_search, #is_archived_search').change(function() {
            ajax_search();
        });
    </script>
    ...
    <script>
        ...
        $(document).on('click', '.load_edit_this_row', function(e) {
            ...
            jQuery.ajax({
                url: '{{ route('dashboard.discounts.load_edit_row') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
        ...
        $(document).on('click', '#do_edit_now', function(e) {
            ...
            var discount_types_id_edit = $("#discount_types_id_edit").val();
            $('#EditModal').modal('hide'); // إخفاء الـ modal

            if (discount_types_id_edit === "") {
                return showWarning('من فضلك أختار نوع الخصومات', '#value_edit');
            }
            $('#EditModal').modal('show'); // إظهار الـ modal مرة أخرى
            ...
            if (total_edit === "") {
                return showWarning('من فضلك ادخل اجمالي الخصومات', '#total_edit');
            }
            $('#EditModal').modal('show'); // إظهار الـ modal مرة أخرى
            ...
            jQuery.ajax({
                url: '{{ route('dashboard.discounts.do_edit_row') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
                    ...
                    id: id,
                    total: total_edit,
                    discount_types_id: discount_types_id_edit,
                    notes: notes_edit,
                    day_price: day_price
                },
                success: function(data) {
