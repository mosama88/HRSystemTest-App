...
@section('allowancesSalary', 'active')
@section('title', 'عرض البدلات المتغيرة الشهر المالى')
@section('css')
...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h5 class="content-title mb-0 my-auto">بيانات البدلات المتغيرة الرواتب للشهر
                    المالى {{ $finance_cln_periods_data->month->name }}</h5><span class="text-muted mt-1 tx-13 mr-2 mb-0">/
                    <a style="font-weight:bold;color:#117ea1;" href="{{ route('dashboard.allowancesSalary.index') }}">
                        <i class="fas fa-arrow-circle-left fa-lg ml-1"></i> سجلات بدلات الموظفين
                    </a>
                </span>
            </div>
        </div>
                ...
                <div class="mb-3 mb-xl-0">
                    <div class="btn-group dropdown">
                        <a class="modal-effect btn btn-outline-primary btn-block" data-effect="effect-super-scaled"
                            data-toggle="modal" href="#modaldemo8"> <i class="fas fa-plus-circle"></i> أنشاء بدل متغير جديد
                            للموظف</a>
                    </div>
                </div>
            </div>
...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-0">
                            بيانات البدل المتغيرة الرواتب للشهر المالى (<span
                ...
                <form method="POST" action="{{ route('dashboard.allowancesSalary.print_search') }}" target="_blank">
                    @csrf
                    ...
                    <div class="col-10 mx-auto mt-4">
                        ...
                        <div class="row">
                            ...
                            <div class=" col-3">
                                <div class="form-group">
                                    <label> بحث بنوع البدل </label>
                                    <select name="allowance_id_search" id="allowance_id_search" class="form-control">
                                        <option value="all">بحث بالكل</option>
                                            ...
                                            <tr>
                                                ...
                                                @if ($finance_cln_periods_data['is_open'] == 1)
                                                    <td>
                                                        ...
                                                        @include('dashboard.salaries.allowancesSalary.delete')
                                                    </td>
                                                @endif
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">أضافة بدل متغير للموظفين للشهر المالى <span class="text-danger">(
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="allowance_id_add" id="allowance_id_add" class="form-control select2"
                                    style="width: 100%;">
    ...
    <div class="modal fade edit" id="EditModal">
        <div class="modal-dialog" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title">تعديل بدل متغير للموظفين للشهر المالى <span class="text-danger">(
                            {{ $finance_cln_periods_data->year_and_month }} )</span> </h6>
                    <button aria-label="Close" class="close" data-dismiss="modal" type="button"><span
                            aria-hidden="true">&times;</span></button>
                </div>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '#do_add_now', function(e) {
                ...
                var allowance_id_add = $("#allowance_id_add").val();
                if (allowance_id_add === "") {
                    $('#modaldemo8').modal('hide'); // إخفاء الـ modal
                    Swal.fire({
                        icon: 'warning',
                        title: 'تحذير',
                        text: 'من فضلك اختر نوع البدل',
                        customClass: {
                            container: 'swal2-override'
                        }
                    }).then((result) => {
                ...
                if (total_add === "" || total_add == 0) {
                    ...
                    Swal.fire({
                        icon: 'warning',
                        title: 'تحذير',
                        text: 'من فضلك ادخل اجمالي البدل للموظف',
                        customClass: {
                            container: 'swal2-override'
                        }
                    }).then((result) => {
                ...
                // عمل فحص قبل التخزين
                $.ajax({
                    url: '{{ route('dashboard.allowancesSalary.checkExsistsBefor') }}',
                    type: 'post',
                    dataType: 'json',
                    cache: false,
                    data: {
                    ...
                    error: function(xhr, status, error) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'خطأ',
                            text: 'عفوا لقد حدث خطأ في التحقق من البدل السابقة',
                            customClass: {
                                container: 'swal2-override'
                            }
                        }).then(() => {
            ...
            function submitAdditional() {
                ...
                $.ajax({
                    url: '{{ route('dashboard.allowancesSalary.store') }}',
                    type: 'post',
                    dataType: 'html',
                    cache: false,
                    data: {
    ...
    <script>
        ...
        function ajax_search() {
            ...
            var allowance_id = $("#allowance_id_search").val();
            var is_archived = $("#is_archived_search").val();
            var the_finance_cln_periods_id = $("#the_finance_cln_periods_id").val();

            jQuery.ajax({
                url: '{{ route('dashboard.allowancesSalary.ajax-search') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
        ...
        $('#employee_code_search, #allowance_id_search, #is_archived_search').change(function() {
            ajax_search();
        });
    </script>
    ...
    <script>
        ...
        $(document).on('click', '.load_edit_this_row', function(e) {
            ...
            jQuery.ajax({
                url: '{{ route('dashboard.allowancesSalary.load_edit_row') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
        ...
        $(document).on('click', '#do_edit_now', function(e) {
            ...
            var allowance_id_edit = $("#allowance_id_edit").val();
            $('#EditModal').modal('hide'); // إخفاء الـ modal

            if (allowance_id_edit === "") {
                return showWarning('من فضلك أختار نوع البدل', '#value_edit');
            }
            $('#EditModal').modal('show'); // إظهار الـ modal مرة أخرى
            ...
            if (total_edit === "") {
                return showWarning('من فضلك ادخل اجمالي البدل', '#total_edit');
            }
            $('#EditModal').modal('show'); // إظهار الـ modal مرة أخرى
            ...
            jQuery.ajax({
                url: '{{ route('dashboard.allowancesSalary.do_edit_row') }}',
                type: 'post',
                dataType: 'html',
                cache: false,
                data: {
                    ...
                    id: id,
                    total: total_edit,
                    allowance_id: allowance_id_edit,
                    notes: notes_edit,
                    day_price: day_price
                },
                success: function(data) {
