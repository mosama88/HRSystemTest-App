    ...
    <div class="row">
        <div class="col-md-6">
            <div class="form-group">
                ...
                <select name="allowance_id_edit" id="allowance_id_edit" class="form-control select2" style="width: 100%;">
                    <option value="">اختر نوع البدل</option>
                    @if (@isset($allowances) && !@empty($allowances))
                        @foreach ($allowances as $all)
                            <option @if ($sanctions['allowance_id'] == $all->id) selected @endif value="{{ $all->id }}">
                                {{ $all->name }}</option>
                        @endforeach
                    @else
                        <div class="alert alert-warning" role="alert" dir="rtl">
