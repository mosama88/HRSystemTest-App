...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">العطلات الرسمية</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/
                    جدول
                    العطلات الرسمية</span>
            </div>
        </div>
        @can('اضافة العطلات الرسمية')
            <div class="d-flex my-xl-auto right-content">
                <div class="mb-3 mb-xl-0">
                    <div class="btn-group dropdown">
                        ...
                        @include('dashboard.settings.holidays.create')
                    </div>
                </div>
            </div>
        @endcan
            ...
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-0">جدول العطلات الرسمية</h4>
                        ...
                        @if (@isset($data) && !@empty($data))
                            <table class="table text-md-nowrap" id="example2">
                                <thead>
                                    <tr>
                                        <th>#</th>
                                        <th class="wd-15p border-bottom-0"> اسم العطلة</th>
                                        <th class="wd-15p border-bottom-0"> من يوم</th>
                                        <th class="wd-15p border-bottom-0"> الى يوم</th>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($data as $info)
                                        ...
                                        <tr>
                                            ...
                                            @include('dashboard.settings.holidays.edit')
                                        </tr>
                                        @include('dashboard.settings.holidays.delete')
                                    @endforeach
                                </tbody>
                            </table>
                        @else
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 422) {
                        ...
                        if (errors.from_date) {
                            $('#from_dateError').text(errors.from_date[0]);
                        }
                        if (errors.to_date) {
                            $('#to_dateError').text(errors.to_date[0]);
                        }
                        if (errors.days_counter) {
                            $('#days_counterError').text(errors.days_counter[0]);
                        }
                        if (errors.active) {
