...
class AttendanceDepartureController extends Controller
{
    ...
    public function show($finance_cln_periods_id)
    {
        ...
        $last_attendance_departures_date = get_cols_where_row_orderby(new AttendanceDepartureActionExcel, ['date_time_action', 'created_at', 'created_by'], ['com_code' => $com_code, 'id' => $finance_cln_periods_id], 'date_time_action', 'DESC');
        ...
        $employees_for_search = get_cols_where(new Employee, ['employee_code', 'name'], ['com_code' => $com_code], 'employee_code', 'ASC');
        $employees = get_cols_where(new Employee, ['employee_code', 'name', 'salary', 'fp_code', 'day_price'], ['com_code' => $com_code], 'employee_code', 'ASC');
    ...
    {
        $com_code = auth()->user()->com_code;
        $finance_cln_periods_data = get_Columns_where_row(new FinanceClnPeriod, ['*'], ['com_code' => $com_code, 'id' => $finance_cln_periods_id, 'is_open' => 1]);
    ...
    {
        $com_code = auth()->user()->com_code;
        $employee_data = get_Columns_where_row(new Employee, ['*'], ['com_code' => $com_code, 'employee_code' => $employee_code]);
    ...
    public function load_fp_archive(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $AttendanceDepartureActionExcel = get_cols_where(new AttendanceDepartureActionExcel, ['*'], ['com_code' => $com_code, 'employee_code' => $request->employee_code, 'finance_cln_periods_id' => $request->finance_cln_periods_id], 'date_time_action', 'ASC');
            if (! empty($AttendanceDepartureActionExcel)) {
                foreach ($AttendanceDepartureActionExcel as $info) {
                    ...
                    $info->week_day_name_ar = get_field_value(new WeekDay, 'name', ['name_en' => $nameOfDay]);
                }
            }
    ...
    public function load_active_attendance_departure(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $other['employee_data'] = get_Columns_where_row(new Employee, ['*'], ['com_code' => $com_code, 'employee_code' => $request->employee_code]);
            $other['VacationType'] = VacationType::all();

            if (! empty($other['employee_data'])) {
                $other['finance_cln_periods_data'] = get_Columns_where_row(new FinanceClnPeriod, ['*'], ['com_code' => $com_code, 'id' => $request->finance_cln_periods_id]);
                if (! empty($other['finance_cln_periods_data'])) {
                    ...
                    if ($other['finance_cln_periods_data']['is_open'] == 1) {
                        ...
                        while ($from_date <= $to_date and $from_date <= $max_attendance_date) {
                            ...
                            $is_exists = get_Columns_where_row(new AttendanceDeparture, ['id'], ['com_code' => $com_code, 'finance_cln_periods_id' => $request->finance_cln_periods_id, 'employee_code' => $request->employee_code, 'the_day_date' => $from_date]);
                            ...
                            if (empty($is_exists)) {
                                ...
                                $main_salary_employees = get_Columns_where_row(new MainSalaryEmployee, ['id'], ['com_code' => $com_code, 'employee_code' => $request->employee_code, 'is_archived' => 0]);
                    ...
                    $other['data'] = get_cols_where(new AttendanceDeparture, ['*'], ['com_code' => $com_code, 'finance_cln_periods_id' => $request->finance_cln_periods_id, 'employee_code' => $request->employee_code], 'the_day_date', 'ASC');
                    $other['total_cut'] = 0;
                    ...
                    if (! empty($other['data'])) {
                        foreach ($other['data'] as $info) {
                            ...
                            $info->week_day_name_ar = get_field_value(new WeekDay, 'name', ['name_en' => $nameOfDay]);
                            $info->attendance_departure_actions_counter = get_count_where(new AttendanceDepartureAction, ['attendance_departure_id' => $info->id, 'finance_cln_periods_id' => $request->finance_cln_periods_id, 'employee_code' => $request->employee_code]);
                    ...
                    if (! empty($other['vacationType_distinct'])) {
                        foreach ($other['vacationType_distinct'] as $info) {
                            $info->name = get_field_value(new VacationType, 'name', ['id' => $info->vacations_types_id]);
                            $info->counter = get_count_where(new AttendanceDeparture, [
                                'com_code' => $com_code,
    ...
    public function load_my_actions(Request $request)
    {
        ...
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $parent = get_Columns_where_row(new AttendanceDeparture, ['id', 'date_time_in', 'date_time_out', 'is_archived', 'is_updated_active_action', 'is_updated_active_action_date', 'is_updated_active_action_by'], ['com_code' => $com_code, 'employee_code' => $request->employee_code, 'id' => $request->attendance_departure_id, 'finance_cln_periods_id' => $request->finance_cln_periods_id]);
            $other['VacationType'] = VacationType::all();
            $AttendanceDepartureAction = get_cols_where(new AttendanceDepartureAction, ['*'], ['com_code' => $com_code, 'employee_code' => $request->employee_code,  'finance_cln_periods_id' => $request->finance_cln_periods_id, 'attendance_departure_id' => $request->attendance_departure_id], 'date_time_action', 'ASC');
    ...
    public function save_active_attendance_departure(Request $request)
    {
        ...
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $attendance_departures = get_Columns_where_row(new AttendanceDeparture, ['the_day_date'], ['com_code' => $com_code, 'id' => $request->id, 'employee_code' => $request->employee_code, 'finance_cln_periods_id' => $request->finance_cln_periods_id, 'is_archived' => 0]);
            if (! empty($attendance_departures)) {
                ...
                $flag = update(new AttendanceDeparture, $dataToUpdate, ['com_code' => $com_code, 'id' => $request->id, 'employee_code' => $request->employee_code, 'finance_cln_periods_id' => $request->finance_cln_periods_id, 'is_archived' => 0]);
                if ($flag) {
    ...
    public function redo_update_actions(Request $request)
    {
        ...
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $attendance_departures = get_Columns_where_row(new AttendanceDeparture, ['*'], ['com_code' => $com_code, 'id' => $request->id, 'employee_code' => $request->employee_code, 'finance_cln_periods_id' => $request->finance_cln_periods_id, 'is_archived' => 0]);
            if (! empty($attendance_departures)) {
                ...
                if ($date_time_out != '') {
                    ...
                    $flagUpdateParent = update(new AttendanceDeparture, $dataUpdate, ['com_code' => $com_code, 'id' => $request->id, 'employee_code' => $request->employee_code, 'finance_cln_periods_id' => $request->finance_cln_periods_id, 'is_archived' => 0]);
                    if ($flagUpdateParent) {
    ...
    {
        $com_code = auth()->user()->com_code;
        $other['employee_data'] = get_Columns_where_row(new Employee, ['*'], ['com_code' => $com_code, 'employee_code' => $employee_code]);
        if (empty($other['employee_data'])) {
        ...
        $other['data'] = get_cols_where(new AttendanceDeparture, ['*'], ['com_code' => $com_code, 'finance_cln_periods_id' => $finance_cln_periods_id, 'employee_code' => $employee_code], 'the_day_date', 'ASC');
        $other['total_cut'] = 0;
        $other['total_attendance_delay'] = 0;
        $other['total_early_departure'] = 0;
        $other['total_hours_total'] = 0;
        ...
        if (! empty($other['data'])) {
            foreach ($other['data'] as $info) {
                $nameOfDay = date('l', strtotime($info->the_day_date));
                $info->week_day_name_ar = get_field_value(new WeekDay, 'name', ['name_en' => $nameOfDay]);
                $info->attendance_departure_actions_counter = get_count_where(new AttendanceDepartureAction, ['attendance_departure_id' => $info->id, 'finance_cln_periods_id' => $finance_cln_periods_id, 'employee_code' => $employee_code]);
    ...
    public function ajax_search(Request $request)
    {
        ...
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $finance_cln_periods_data = get_Columns_where_row(new FinanceClnPeriod, ['*'], ['com_code' => $com_code, 'id' => $request->finance_cln_periods_id]);
            $employee_code = $request->employee_code;
            ...
            if (! empty($data)) {
                foreach ($data as $info) {
                    ...
                    $info->emp_fp_code = get_field_value(new Employee, 'fp_code', ['com_code' => $com_code]);
                }
            }
