...
class EmployeeSalaryRewardsController extends Controller
{
    ...
    {
        $this->middleware('permission:المكافئات', ['only' => ['index']]);
        $this->middleware('permission:عرض المكافئات', ['only' => ['show']]);
        $this->middleware('permission:اضافة المكافئات', ['only' => ['create', 'store']]);
        $this->middleware('permission:تعديل المكافئات', ['only' => ['update', 'edit']]);
        $this->middleware('permission:حذف المكافئات', ['only' => ['destroy']]);
        $this->middleware('permission:طباعه المكافئات', ['only' => ['print_search']]);
    }
    ...
    public function index()
    {
        ...
        return view('dashboard.salaries.rewards.index', compact('data'));
    }
    ...
    public function store(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (!empty($finance_cln_periods_data) and !empty($main_salary_employee_data)) {
                ...
                $dataToInsert['additional_types_id'] = $request->additional_types_id;
                $dataToInsert['total'] = $request->total;
                $dataToInsert['notes'] = $request->notes;
                $dataToInsert['created_by'] = auth()->user()->id;
                $dataToInsert['com_code'] = $com_code;
                $flagInsert = insert(new EmployeeSalaryRewards(), $dataToInsert);
    ...
    public function show($finance_cln_periods_id)
    {
        ...
        $data = EmployeeSalaryRewards::orderBy('id', 'DESC')
        ...
        $additionals = AdditionalType::where("com_code", "=", $com_code)->get();


        return view('dashboard.salaries.rewards.show', compact('data', 'finance_cln_periods_data', 'employees', 'employees_for_search', 'additionals'));
    }
    ...
    public function load_edit_row(Request $request)
    {
        if ($request->ajax()) {
            ...
            $sanctions = get_Columns_where_row(new EmployeeSalaryRewards(), array("*"), array("com_code" => $com_code, 'id' => $request->id, 'is_archived' => 0, 'finance_cln_periods_id' => $request->the_finance_cln_periods_id, 'main_salary_employees_id' => $request->main_salary_employees_id));
            ...
            return view('dashboard.salaries.rewards.load_edit_row', ['finance_cln_periods_data' => $finance_cln_periods_data, 'main_salary_employees_data' => $main_salary_employees_data, 'sanctions' => $sanctions, 'employees' => $employees, 'employee' => $employee, 'additionals' => $additionals]);
        }
    }
    ...
    public function do_edit_row(Request $request)
    {
        if ($request->ajax()) {
            ...
            $sanctions = EmployeeSalaryRewards::where([
                'com_code' => $com_code,
                'id' => $request->id,
                'is_archived' => 0,
                'finance_cln_periods_id' => $request->the_finance_cln_periods_id,
            ...
            if ($finance_cln_periods_data && $main_salary_employees && $sanctions) {
                ...
                try {
                    ...
                    $flagInsert = $sanctions->update([
                        ...
                        'additional_types_id' => $request->additional_types_id,
                        'total' => $request->total,
                        'notes' => $request->notes,
    ...
    public function destroy($id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $data = get_Columns_where_row(new EmployeeSalaryRewards(), array("*"), array("com_code" => $com_code, 'id' => $id));
            if (empty($data)) {
            ...
            DB::beginTransaction();
            $flagInsert = destroy(new EmployeeSalaryRewards(), array("com_code" => $com_code, 'id' => $id));
            if ($flagInsert) {
    ...
    public function checkExsistsBefor(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $checkExsistsBeforCounter = get_count_where(new EmployeeSalaryRewards(), array("com_code" => $com_code, "finance_cln_periods_id" => $request->the_finance_cln_periods_id, "employee_code" => $request->employee_code));
    ...
    public function ajaxSearch(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $com_code = auth()->user()->com_code;
            $data = EmployeeSalaryRewards::select("*")->where($field1, $operator1, $value1)->where($field2, $operator2, $value2)->where($field3, $operator3, $value3)
            ...
            return view('dashboard.salaries.rewards.ajax_search', ['data' => $data]);
        }
    }
    ...
    public function print_search(Request $request)
    {
        ...
        $additional_types_id = $request->additional_types_id_search;
        ...
        return view('dashboard.salaries.rewards.print_search', ['data' => $data, 'finance_cln_periods_data' => $finance_cln_periods_data, 'systemData' => $systemData, 'other' => $other]);
    }
}