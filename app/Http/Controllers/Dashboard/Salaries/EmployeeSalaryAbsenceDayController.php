...
class EmployeeSalaryAbsenceDayController extends Controller
{
    ...
    {
        $this->middleware('permission:الغيابات', ['only' => ['index']]);
        $this->middleware('permission:عرض الغيابات', ['only' => ['show']]);
        $this->middleware('permission:اضافة الغيابات', ['only' => ['create', 'store']]);
        $this->middleware('permission:تعديل الغيابات', ['only' => ['update', 'edit']]);
        $this->middleware('permission:حذف الغيابات', ['only' => ['destroy']]);
        $this->middleware('permission:طباعه الغيابات', ['only' => ['print_search']]);
    }
    ...
    public function store(Request $request)
    {
        if ($request->ajax()) {
            ...
            if (!empty($finance_cln_periods_data) and !empty($main_salary_employee_data)) {
                ...
                $flagInsert =  insert(new EmployeeSalaryAbsenceDay(), $dataToInsert);
                if ($flagInsert) {
    ...
    public function show($finance_cln_periods_id)
    {
        ...
        $data = EmployeeSalaryAbsenceDay::orderBy('id', 'DESC')
        ...
        return view('dashboard.salaries.absences.show', compact('data', 'finance_cln_periods_data', 'employees', 'employees_for_search'));
    }
    ...
    public function load_edit_row(Request $request)
    {
        if ($request->ajax()) {
            ...
            $sanctions = get_Columns_where_row(new EmployeeSalaryAbsenceDay(), array("*"), array("com_code" => $com_code, 'id' => $request->id, 'is_archived' => 0, 'finance_cln_periods_id' => $request->the_finance_cln_periods_id, 'main_salary_employees_id' => $request->main_salary_employees_id));
            ...
            return view('dashboard.salaries.absences.load_edit_row', ['finance_cln_periods_data' => $finance_cln_periods_data, 'main_salary_employees_data' => $main_salary_employees_data, 'sanctions' => $sanctions, 'employees' => $employees, 'employee' => $employee]);
        }
    }
    ...
    public function do_edit_row(Request $request)
    {
        if ($request->ajax()) {
            ...
            $sanctions = EmployeeSalaryAbsenceDay::where([
                'com_code' => $com_code,
                'id' => $request->id,
                'is_archived' => 0,
                'finance_cln_periods_id' => $request->the_finance_cln_periods_id,
    ...
    public function destroy($id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $data = get_Columns_where_row(new EmployeeSalaryAbsenceDay(), array("*"), array("com_code" => $com_code, 'id' => $id));
            ...
            DB::beginTransaction();
            $flagInsert =  destroy(new EmployeeSalaryAbsenceDay(), array("com_code" => $com_code, 'id' => $id));
    ...
    public function checkExsistsBefor(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $checkExsistsBeforCounter = get_count_where(new EmployeeSalaryAbsenceDay(), array("com_code" => $com_code, "finance_cln_periods_id" => $request->the_finance_cln_periods_id, "employee_code" => $request->employee_code));
    ...
    public function ajaxSearch(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $com_code = auth()->user()->com_code;
            $data = EmployeeSalaryAbsenceDay::select("*")->where($field1, $operator1, $value1)->where($field3, $operator3, $value3)
            ...
            return view('dashboard.salaries.absences.ajax_search', ['data' => $data]);
        }
    }
    ...
    public function print_search(Request $request)
    {
        ...
        return view('dashboard.salaries.absences.print_search', ['data' => $data, 'finance_cln_periods_data' => $finance_cln_periods_data, 'systemData' => $systemData, 'other' => $other]);
    }
}
