...
class EmployeeSalaryAbsenceDayController extends Controller
{
    ...
    public function load_edit_row(Request $request)
    {
        if ($request->ajax()) {
            ...
            $sanctions = get_Columns_where_row(new EmployeeSalaryAbsenceDay, ['*'], ['com_code' => $com_code, 'id' => $request->id, 'is_archived' => 0, 'finance_cln_periods_id' => $request->the_finance_cln_periods_id, 'main_salary_employees_id' => $request->main_salary_employees_id]);
    ...
    public function destroy($id)
    {
        try {
            $com_code = auth()->user()->com_code;
            $data = get_Columns_where_row(new EmployeeSalaryAbsenceDay, ['*'], ['com_code' => $com_code, 'id' => $id]);
            ...
            DB::beginTransaction();
            $flagInsert = destroy(new EmployeeSalaryAbsenceDay, ['com_code' => $com_code, 'id' => $id]);
    ...
    public function checkExsistsBefor(Request $request)
    {
        if ($request->ajax()) {
            $com_code = auth()->user()->com_code;
            $checkExsistsBeforCounter = get_count_where(new EmployeeSalaryAbsenceDay, ['com_code' => $com_code, 'finance_cln_periods_id' => $request->the_finance_cln_periods_id, 'employee_code' => $request->employee_code]);
