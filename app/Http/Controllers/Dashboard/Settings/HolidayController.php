...
class HolidayController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = getColumnsIndex(new Holiday(), array("*"), array("com_code" => $com_code), 'id', 'DESC')->get();
    ...
    public function store(HolidayRequest $request)
    {
        try {
            $com_code = auth()->user()->com_code;
            $checkExsists = get_Columns_where_row(new Holiday(), array("id"), array("com_code" => $com_code, "name" => $request->name));
...
*/
    ...
    {
        $com_code = auth()->user()->com_code;
        $data = get_Columns_where_row(new Holiday(), array("*"), array("com_code" => $com_code, "id" => $id));
        if (empty($data)) {
    ...
    public function update($id, HolidayRequest $request)
    {
        try {
            ...
            update(new Holiday(), $dataToUpdate, array("com_code" => $com_code, "id" => $id));
            DB::commit();
    ...
    public function destroy($id)
    {
        try {
            ...
            DB::beginTransaction();
            destroy(new Holiday(), array("com_code" => $com_code, "id" => $id));
            DB::commit();
