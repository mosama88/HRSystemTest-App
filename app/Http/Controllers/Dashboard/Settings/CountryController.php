...
class CountryController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = getColumnsIndex(new Country, ['*'], ['com_code' => $com_code], 'id', 'DESC')->get();
        if (! empty($data)) {
    ...
    public function store(CountryRequest $request)
    {
        try {
            ...
            $com_code = auth()->user()->com_code;
            $CheckExsists = get_Columns_where_row(new Country, ['id'], ['com_code' => $com_code, 'name' => $request->name]);
    ...
    public function edit($id)
    {
        ...
        $com_code = auth()->user()->com_code;
        $data = get_Columns_where_row(new Country, ['*'], ['com_code' => $com_code, 'id' => $id]);
        if (empty($data)) {
    ...
    public function update($id, CountryRequest $request)
    {
        try {
            ...
            update(new Country, $DataToUpdate, ['com_code' => $com_code, 'id' => $id]);
            DB::commit();
    ...
    public function destroy($id)
    {
        try {
            ...
            $counterUsed = get_count_where(new Employee, ['com_code' => $com_code, 'country_id' => $id]);
            if ($counterUsed > 0) {
                return redirect()->route('dashboard.countries.index')->with(['error' => 'عفوآ غير قادر على الحذف لانه قد تم أستخدامه من قبل']);
            }
            DB::beginTransaction();
            destroy(new Country, ['com_code' => $com_code, 'id' => $id]);
            DB::commit();
