...
class CityController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = getColumnsIndex(new City, ['*'], ['com_code' => $com_code], 'id', 'DESC')->get();
        if (! empty($data)) {
    ...
    public function store(CityRequest $request)
    {
        try {
            ...
            $com_code = auth()->user()->com_code;
            $CheckExsists = get_Columns_where_row(new City, ['id'], ['com_code' => $com_code, 'name' => $request->name]);
    ...
    public function edit($id)
    {
        ...
        $com_code = auth()->user()->com_code;
        $data = get_Columns_where_row(new City, ['*'], ['com_code' => $com_code, 'id' => $id]);
        if (empty($data)) {
    ...
    public function update($id, CityRequest $request)
    {
        try {
            ...
            update(new City, $DataToUpdate, ['com_code' => $com_code, 'id' => $id]);
            DB::commit();
    ...
    public function destroy($id)
    {
        try {
            ...
            $counterUsed = get_count_where(new Employee, ['com_code' => $com_code, 'city_id' => $id]);
            if ($counterUsed > 0) {
                return redirect()->route('dashboard.cities.index')->with(['error' => 'عفوآ غير قادر على الحذف لانه قد تم أستخدامه من قبل']);
            }
            DB::beginTransaction();
            destroy(new City, ['com_code' => $com_code, 'id' => $id]);
            DB::commit();
    ...
    {
        $data = getColumnsIndex(new Governorate, ['*'], [], 'id', 'DESC')->get();
