...
class NationalityController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = getColumnsIndex(new Nationality(), array("*"), array("com_code" => $com_code), 'id', 'DESC')->get();
        if (!empty($data)) {
    ...
    public function store(NationalityRequest $request)
    {
        try {
            ...
            $com_code = auth()->user()->com_code;
            $CheckExsists = get_Columns_where_row(new Nationality(), array("id"), array("com_code" => $com_code, 'name' => $request->name));
    ...
    public function edit($id)
    {
        ...
        $com_code = auth()->user()->com_code;
        $data = get_Columns_where_row(new Nationality(), array("*"), array("com_code" => $com_code, 'id' => $id));
        if (empty($data)) {
    ...
    public function destroy($id)
    {
        try {
            ...
            $counterUsed = get_count_where(new Employee(), array("com_code" => $com_code, "nationality_id" => $id));
            if ($counterUsed > 0) {
