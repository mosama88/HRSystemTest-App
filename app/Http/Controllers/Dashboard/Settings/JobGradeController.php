...
class JobGradeController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = getColumnsIndex(new JobGrade(), array("*"), array('com_code' => $com_code), 'id', 'DESC')->get();
        if (!empty($data)) {
    ...
    public function store(JobGradeRequest $request)
    {
        try {
            $com_code = auth()->user()->com_code;
            $CheckExsists = get_Columns_where_row(new JobGrade(), array("id"), array("name" => $request->name, 'com_code' => $com_code));
    ...
    {
        $com_code = auth()->user()->com_code;
        $data = get_Columns_where_row(new JobGrade(), array("*"), array("com_code" => $com_code, 'id' => $id));
        if (empty($data)) {
    ...
    public function update($id, JobGradeRequest $request)
    {
        ...
        try {
            ...
            update(new JobGrade(), $dataToUpdate, array("com_code" => $com_code, 'id' => $id));
            DB::commit();
    ...
    public function destroy($id)
    {
        try {
            ...
            $counterUsed = get_count_where(new Employee(), array("com_code" => $com_code, "job_grade_id" => $id));
            if ($counterUsed > 0) {
                return redirect()->route('dashboard.jobGrades.index')->with(['error' => 'عفوآ غير قادر على الحذف لانه قد تم أستخدامه من قبل']);
            }

            DB::beginTransaction();
            destroy(new JobGrade(), array("com_code" => $com_code, 'id' => $id));
            DB::commit();
