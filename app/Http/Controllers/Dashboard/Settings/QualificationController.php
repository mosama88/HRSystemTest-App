...
class QualificationController extends Controller
{
    ...
    public function index()
    {
        ...
        $data = getColumnsIndex(new Qualification(), array("*"), array('com_code' => $com_code), 'id', 'DESC')->get();
        if (!empty($data)) {
    ...
    public function store(QualificationRequest $request)
    {
        try {
            $com_code = auth()->user()->com_code;
            $checkExsists = get_Columns_where_row(new Qualification(), array("id"), array("name" => $request->name, "com_code" => $com_code));
    ...
    {
        $com_code = auth()->user()->com_code;
        $data = get_Columns_where_row(new Qualification(), array("*"), array("com_code" => $com_code, 'id' => $id));
        if (empty($data)) {
    ...
    public function destroy($id)
    {
        try {
            ...
            $counterUsed = get_count_where(new Employee(), array("com_code" => $com_code, "qualification_id" => $id));
            if ($counterUsed > 0) {
