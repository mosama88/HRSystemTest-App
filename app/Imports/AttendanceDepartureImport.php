...
class AttendanceDepartureImport implements ToCollection
{
    ...
    public function collection(Collection $rows)
    {
        ...
        $dataSetting = get_Columns_where_row(new AdminPanelSetting, ['*'], ['com_code' => $com_code]);
        ...
        foreach ($rows as $row) {
            ...
            $employee_data = get_Columns_where_row(new Employee, ['employee_code', 'has_fixed_shift', 'shift_types_id', 'daily_work_hour', 'branch_id', 'functional_status'], ['com_code' => $com_code, 'fp_code' => $row[2]]);
            if (! empty($employee_data)) {

                $checkExistsBefore = get_Columns_where_row(new AttendanceDepartureActionExcel, ['id'], ['com_code' => $com_code, 'finance_cln_periods_id' => $this->finance_cln_periods_data['id'], 'employee_code' => $employee_data['employee_code'], 'date_time_action' => date('Y-m-d H:i:s', strtotime($row[3])), 'action_type' => $action_type]);
                $checkExistsBeforeSalary = get_Columns_where_row(new MainSalaryEmployee, ['id'], ['com_code' => $com_code, 'finance_cln_periods_id' => $this->finance_cln_periods_data['id'], 'employee_code' => $employee_data['employee_code']]);
                ...
                if (empty($checkExistsBefore)) {
                    ...
                    if ($employee_data['has_fixed_shift'] == 'Yes') {
                        $shift_data = get_Columns_where_row(new ShiftsType, ['from_time', 'to_time', 'total_hours'], ['com_code' => $com_code, 'id' => $employee_data['shift_types_id']]);
                        if (empty($shift_data)) {
                            continue;
                        } else {
                    ...
                    $check_for_empty_recored = get_Columns_where_row(new AttendanceDeparture, ['id'], ['com_code' => $com_code, 'employee_code' => $employee_data['employee_code'], 'finance_cln_periods_id' => $this->finance_cln_periods_data['id'], 'the_day_date' => date('Y-m-d', strtotime($row[3])), 'date_time_in' => null]);
                    if (! empty($check_for_empty_recored)) {
                        // لو البصمة الحالية حضور  نقوم بحذف البصمة الفاضية
                        if ($action_type == 1) {
                            destroy(new AttendanceDeparture, ['id' => $check_for_empty_recored['id']]);
                        }
                    }
                    ...
                    if (! empty($last)) {
                        ...
                        if ($last['date_in'] == date('Y-m-d', strtotime($row[3]))) {
                            ...
                            if ($minuitdiff > $dataSetting['less_than_minute_neglecting_fp']) {
                                ...
                                if ($employee_data['has_fixed_shift'] == 'Yes') {
                                    if ($shift_data['to_time'] > $timeEnter) {
                                        ...
                                        if ($minutesIfNow >= $dataSetting['after_minute_calculate_delay']) {
                                            ...
                                            $counterCutQuarterDay = get_count_where(new AttendanceDeparture, ['com_code' => $com_code, 'employee_code' => $employee_data['employee_code'], 'finance_cln_periods_id' => $this->finance_cln_periods_data['id'], 'cut' => .25]);
                                            $counterCutHalfDay = get_count_where(new AttendanceDeparture, ['com_code' => $com_code, 'employee_code' => $employee_data['employee_code'], 'finance_cln_periods_id' => $this->finance_cln_periods_data['id'], 'cut' => .5]);
                                            $counterCutOneDay = get_count_where(new AttendanceDeparture, ['com_code' => $com_code, 'employee_code' => $employee_data['employee_code'], 'finance_cln_periods_id' => $this->finance_cln_periods_data['id'], 'cut' => 1]);
                                ...
                                $flagUpdateParent = update(new AttendanceDeparture, $dataUpdate, ['id' => $last['id'], 'com_code' => $com_code]);
                                if ($flagUpdateParent) {
                                    ...
                                    update(new AttendanceDepartureAction, $dataToUpdateAction, ['com_code' => $com_code, 'action_type' => $action_type, 'attendance_departure_id' => $last['id'], 'date_time_action' => $dataUpdate['date_time_out']]);
                                }
                            }
                        } else {
                            ...
                            if ($hourdiff <= $shift_hours) {
                                ...
                                if (($hourdiff - $shift_hours) <= $dataSetting['max_hours_take_fp_as_addtional']) {
                                    ...
                                    $main_salary_employees = get_Columns_where_row(new MainSalaryEmployee, ['id'], ['com_code' => $com_code, 'employee_code' => $employee_data['employee_code'], 'is_archived' => 0]);
                                    ...
                                    if ($flagInsertParent) {
                                        ...
                                        insert(new AttendanceDepartureAction, $dataInsertAction);
                                        $dataToUpdateAction['is_active_with_parent'] = 1;
                                        update(new AttendanceDepartureAction, $dataToUpdateAction, ['com_code' => $com_code, 'action_type' => $action_type, 'attendance_departure_id' => $flagInsertParent['id'], 'date_time_action' => $datainsert['date_time_in']]);
                                    }
                                }
                            }
                        }
