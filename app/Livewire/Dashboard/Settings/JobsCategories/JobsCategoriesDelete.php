...
class JobsCategoriesDelete extends Component
{
    ...
    public function submit(){
        ...
        $data = get_Columns_where_row(new JobsCategory(), array("*"), array("com_code" => $com_code, 'id' => $this->dataDeleteRecored->id));
        if (empty($data)) {
            return redirect()->route('dashboard.jobsCategories.index')->with(['error' => 'عفوا غير قادر الي الوصول الي البيانات المطلوبة']);
        }
        $counterUsed = get_count_where(new Employee(), array("com_code" => $com_code, "job_categories_id" => $this->dataDeleteRecored->id));
        if ($counterUsed > 0) {
