...
class BranchDelete extends Component
{
    ...
    public function submit()
    {
        ...
        $data = get_Columns_where_row(new Branch, ['*'], ['id' => $this->branchDeleteRecored->id, 'com_code' => $com_code]);
        if (empty($data)) {
            return redirect()->route('dashboard.branches.index')->withErrors(['error' => 'عفوآ غير قادر على الوصول للبيانات المطلوبه']);
        }
        $counterUsed = get_count_where(new Employee, ['com_code' => $com_code, 'branch_id' => $this->branchDeleteRecored->id]);
        if ($counterUsed > 0) {
