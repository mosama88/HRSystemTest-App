...
class CitiesDelete extends Component
{
    ...
    public function submit(){
        ...
        $data = get_Columns_where_row(new City(), array("*"), array("com_code" => $com_code, 'id' => $this->dataDeleteRecored->id));
        if (empty($data)) {
            return redirect()->route('dashboard.cities.index')->with(['error' => 'عفوا غير قادر للوصول الي البيانات المطلوبة']);
        }
        $counterUsed = get_count_where(new Employee(), array("com_code" => $com_code, "city_id" => $this->dataDeleteRecored->id));
        if ($counterUsed > 0) {
