...
class AllowancesDelete extends Component
{
    ...
    public function submit()
    {
        ...
        $data = get_Columns_where_row(new Allowance, ['*'], ['com_code' => $com_code, 'id' => $this->dataDeleteRecored->id]);
        if (empty($data)) {
            return redirect()->route('dashboard.allowances.index')->with(['error' => 'عفوا غير قادر للوصول الي البيانات المطلوبة']);
        }

        $counterUsed = get_count_where(new EmployeeFixedAllowance, ['com_code' => $com_code, 'allowance_id' => $this->dataDeleteRecored->id]);
