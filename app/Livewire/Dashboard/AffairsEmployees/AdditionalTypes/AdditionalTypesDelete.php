...
class AdditionalTypesDelete extends Component
{
    ...
    public function submit()
    {
        ...
        $data = get_Columns_where_row(new AdditionalType, ['*'], ['com_code' => $com_code, 'id' => $this->dataDeleteRecored->id]);
        if (empty($data)) {
            return redirect()->route('dashboard.additional_types.index')->with(['error' => 'عفوا غير قادر للوصول الي البيانات المطلوبة']);
        }
        $counterUsed = get_count_where(new EmployeeSalaryRewards, ['com_code' => $com_code, 'additional_types_id' => $this->dataDeleteRecored->id]);
        if ($counterUsed > 0) {
