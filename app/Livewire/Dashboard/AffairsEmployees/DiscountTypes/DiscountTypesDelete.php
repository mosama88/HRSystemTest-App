...
class DiscountTypesDelete extends Component
{
    ...
    public function submit()
    {
        ...
        $data = get_Columns_where_row(new DiscountType, ['*'], ['com_code' => $com_code, 'id' => $this->dataDeleteRecored->id]);
        if (empty($data)) {
            return redirect()->route('dashboard.discount_types.index')->with(['error' => 'عفوا غير قادر للوصول الي البيانات المطلوبة']);
        }

        $counterUsed = get_count_where(new EmployeeSalaryDiscount, ['com_code' => $com_code, 'discount_types_id' => $this->dataDeleteRecored->id]);
        if ($counterUsed > 0) {
